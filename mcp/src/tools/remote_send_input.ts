...
export async function remoteSendInput(
  pool: DaemonPool,
  args: { server: string; sessionId: string; input: string }
): Promise<unknown> {
  const client = pool.get(args.server);
  await client.ensureConnected();
  client.requireCapability('dispatch');

  const response = await client.sendRequest<DaemonResponse>('send_input', {
  ...
  if (!response.success) {
    ...
    throw new DaemonRequestFailed('send_input', message);
  }
